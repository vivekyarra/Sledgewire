...
- `npm test`: **90/90** automated tests passed.
- `npm run selfcheck`: signed receipt verified across clean and hostile fixture families.
- `npm run stress -- 2000 64`: **2,000/2,000** complete smoke workflows succeeded, 0 infrastructure failures. One local run completed in **4.662 s**, p50 **130 ms**, p95 **210 ms**, p99 **724 ms**.
...
3. Response limits previously applied after buffering. v0.2 enforces the byte ceiling while streaming.
4. MCP sessions were previously stateless one-call helpers. v0.2 carries `mcp-session-id`, protocol version and initialized notification through a workflow.
5. Target-declared tool errors could look successful. `isError: true` now fails the probe/invocation.
6. Adversarial replay could duplicate a side effect. Replay/invalid-argument calls are attempted only on explicitly safe/read-only/idempotent tools; destructive hints fail closed.
...
10. SharedOS was architecture-only. v0.2 uses the current `@aicoo/sharedos` package path, atomic bounded-use storage, durable audit/outbox, exact dynamic requirements and allow/deny CI checks. `invoke` uses separated Scout/Mechanic/Inspector/Breaker stages.
11. SharedNet was runbook-only. v0.2 includes the Room watcher command, native ledger reader, payment-required response, exact memo binding and replay-safe provider handler.

## Hostile fixture coverage

Clean, hostile output, hostile description, fake-success envelope, malformed JSON, oversized output, replay-sensitive service, destructive tool, unknown-tool acceptance, forbidden-extra-field acceptance, divergent idempotent replay, Streamable HTTP/SSE response, redirect attempt, deadline breach and huge tool catalog.
...
5. the SharedOS event integration supplied for the event, if entering that track, with a real Sledgewire trace visible;
