...
4. Deploy the persistent seller topology. For a one-service PaaS, use the checked-in Railway config and `npm run arena:all`; for a Docker host, use `compose.arena.yml`.
...
6. Start the seller against the persistent database and signing key. Wait for `/ready` to report a fresh daemon heartbeat.
