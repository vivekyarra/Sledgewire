...
Breaker    invoke exact approved target/tool
```
...
- DNS is resolved first and the validated address is pinned into the actual HTTP/TLS connection; redirect following is disabled.
- Loopback, private, link-local, documentation, benchmark and multicast ranges are denied by default.
