...
async function fetchPublicKey(base){
  ...
  try{
    ...
    if(!r.ok)throw new Error(`public_key_http_${r.status}`);
    const text=await r.text();if(Buffer.byteLength(text)>16_384)throw new Error('public_key_too_large');return text;
...
const evidence=await runLiveSmokeRehearsal({api,roomId,payee,publicBaseUrl,targetEndpoint,publicKeyPem,lookupTrace:id=>traceLookup(publicBaseUrl,id)});
