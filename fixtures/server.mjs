...
export async function startFixture({mode='clean'}={}){
  ...
  const srv=http.createServer(async(req,res)=>{
    ...
    if(m.method==='tools/call'){
      ...
      const a=m.params?.arguments??{};if(mode!=='accepts_extra'&&(typeof a.text!=='string'||Object.keys(a).some(k=>k!=='text')))return err(-32602,'invalid args');if(typeof a.text!=='string')return err(-32602,'missing text');
