...
test('live evidence validator requires signed request-scoped payment and trace proof',()=>{
  ...
  const r=validateLiveRehearsalEvidence(live,{roomId:room,publicBaseUrl:base,publicKeyPem:kp.publicKeyPem});
  assert.equal(r.ok,true);
  const tampered=structuredClone(live);tampered.receipt.payment.price_credits=35;
  assert.equal(validateLiveRehearsalEvidence(tampered,{roomId:room,publicBaseUrl:base,publicKeyPem:kp.publicKeyPem}).ok,false);
});
