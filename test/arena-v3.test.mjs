...
test('quote marks incomplete templates instead of presenting them as send-ready',()=>{
  const q=quote({intent:'preflight'});assert.equal(q.request_ready,false);assert.deepEqual(q.missing_fields,['endpoint']);assert.equal(q.input_validation.reason,'missing_required_quote_context');
  const invoke=quote({intent:'repair_execute',endpoint:'https://example.com/mcp'});assert.equal(invoke.request_ready,false);assert.ok(invoke.missing_fields.includes('tool'));
});
test('quote marks structurally valid paid templates send-ready',()=>{
  const smoke=quote({intent:'preflight',endpoint:'https://example.com/mcp'});assert.equal(smoke.request_ready,true);assert.deepEqual(smoke.missing_fields,[]);
  const fleet=quote({intent:'compare',targets:[{endpoint:'https://example.com/mcp'}]});assert.equal(fleet.request_ready,true);
});
