...
test('Arena stats aggregate credits buyers mix conversion latency and evidence without identities',()=>{
  ...
  const x=s.arenaStats();
  assert.equal(x.earned_credits,36);assert.equal(x.unique_buyers,2);assert.equal(x.paid_transactions,3);assert.equal(x.failed_requests,1);assert.equal(x.service_mix['sledgewire.smoke'],1);assert.equal(x.outcome_mix.DEGRADED,1);
  ...
  assert.equal(x.integrity.signed_deliveries,3);assert.equal(x.integrity.trace_bound_deliveries,3);assert.equal(JSON.stringify(x).includes('i_BUYER'),false);
});
test('Arena stats are safe and zero-valued on an empty store',()=>{
  const x=new ArenaStore(':memory:').arenaStats();assert.equal(x.earned_credits,0);assert.equal(x.unique_buyers,0);assert.equal(x.delivery_ms.p95,null);assert.equal(x.smoke_to_premium_conversion,0);
});
