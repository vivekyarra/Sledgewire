...
function completed(store,{id,txn,service,buyer,price,start='2026-09-25T00:00:00.000Z',end='2026-09-25T00:00:01.000Z',outcome='READY'}){
  store.db.prepare("INSERT INTO requests(request_id,txn_id,fingerprint,service,buyer_seat,status,response_json,started_at,completed_at) VALUES(?,?,?,?,?, 'completed',?,?,?)").run(id,txn,'fp-'+id,service,buyer,JSON.stringify({state:'DELIVERED',outcome_state:outcome,trace_id:'trace-'+id,receipt:{buyer_seat:buyer,sharedos_trace_id:'trace-'+id,payment:{txn_id:txn,price_credits:price},proof:{signature:'sig'}}}),start,end);
}
...
test('Arena gross earned credits include verified paid claims that later fail execution',()=>{
  const s=new ArenaStore(':memory:');
  s.db.prepare("INSERT INTO requests(request_id,txn_id,fingerprint,service,buyer_seat,status,error_json,started_at,completed_at) VALUES('rf2','tf2','fpf2','sledgewire.gauntlet','i_BUYERDDDD','failed','{}','2026-09-25T00:00:00.000Z','2026-09-25T00:00:01.000Z')").run();
  const x=s.arenaStats({prices:{'sledgewire.gauntlet':35}});
  assert.equal(x.earned_credits,35);assert.equal(x.unique_buyers,1);assert.equal(x.paid_transactions,1);assert.equal(x.failed_requests,1);assert.equal(x.paid_service_mix['sledgewire.gauntlet'],1);assert.equal(x.integrity.unknown_price_claims,0);
});
