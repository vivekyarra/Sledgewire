...
test('Arena stats aggregate credits buyers mix conversion latency and evidence without identities',()=>{
  ...
  assert.equal(x.earned_credits,39);assert.equal(x.unique_buyers,2);assert.equal(x.paid_transactions,4);assert.equal(x.failed_requests,1);assert.equal(x.service_mix['sledgewire.smoke'],1);assert.equal(x.outcome_mix.DEGRADED,1);
