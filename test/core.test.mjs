...
test('invoke executes clean explicitly requested tool',async()=>{const f=await startFixture();try{const r=await invoke(f.url,{name:'safe_echo',arguments:{text:'hello'}},{targetPolicy:tp});assert.equal(r.state,'READY');assert.ok(r.result_sha256);}finally{await f.close();}});
...
test('invoke downgrades suspicious untrusted tool metadata even when result is clean',async()=>{const f=await startFixture({mode:'description_injection'});try{const r=await invoke(f.url,{name:'safe_echo',arguments:{text:'hello'}},{targetPolicy:tp});assert.equal(r.state,'DEGRADED');assert.equal(r.untrusted_tool_metadata,true);}finally{await f.close();}});
test('invoke blocks destructive tool without explicit destructive authority',async()=>{const f=await startFixture({mode:'destructive'});try{const r=await invoke(f.url,{name:'safe_echo',arguments:{text:'hello'}},{targetPolicy:tp});assert.equal(r.state,'BLOCKED');assert.equal(r.reason,'destructive_tool_requires_explicit_authority');}finally{await f.close();}});
test('invoke permits destructive tool only with explicit destructive authority',async()=>{const f=await startFixture({mode:'destructive'});try{const r=await invoke(f.url,{name:'safe_echo',arguments:{text:'hello'},authorizeDestructive:true},{targetPolicy:tp});assert.equal(r.state,'READY');}finally{await f.close();}});
...
for(const [name,value,s,ok] of [
 ...
 ['nullable union null',null,{type:['string','null']},true],['nullable union string','x',{type:['string','null']},true],['nullable union wrong',4,{type:['string','null']},false],
 ['$schema metadata accepted',{x:'a'},{$schema:'https://json-schema.org/draft/2020-12/schema',type:'object',properties:{x:{type:'string'}},additionalProperties:false},true],
