...
test('server/discover advertises modern plus legacy and identity only in _meta',async()=>{
  ...
  assert.equal(r.result.resultType,'complete');assert.ok(r.result.supportedVersions.includes('2026-07-28'));assert.ok(r.result.supportedVersions.includes('2025-11-25'));assert.equal(r.result.serverInfo,undefined);assert.equal(r.result._meta['io.modelcontextprotocol/serverInfo'].version,'0.3.6');
});
