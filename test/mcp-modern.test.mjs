...
test('client negotiates modern 2026-07-28 and sends per-request metadata',async()=>{
  ...
  try{
    ...
    for(const x of [discover,list,call]){assert.equal(x.headers.protocol,MODERN_PROTOCOL_VERSION);assert.equal(x.meta['io.modelcontextprotocol/protocolVersion'],MODERN_PROTOCOL_VERSION);assert.deepEqual(x.meta['io.modelcontextprotocol/clientCapabilities'],{});assert.equal(x.headers.session,null);}
    assert.equal(call.headers.mcpMethod,'tools/call');assert.equal(call.headers.mcpName,'safe_echo');
...
test('modern HTTP requires matching protocol header and body metadata',()=>{
  ...
  for(const headers of [{},{'mcp-protocol-version':'2025-11-25'}]){const r=validateHttpMcp(msg,headers);assert.equal(r.ok,false);assert.equal(r.body.error.code,-32020);}
});
test('modern HTTP rejects method name and removed session header mismatches',()=>{
  ...
  assert.equal(validateHttpMcp(msg,{...base,'mcp-method':'tools/list'}).body.error.code,-32020);
  assert.equal(validateHttpMcp(msg,{...base,'mcp-method':'tools/call','mcp-name':'wrong'}).body.error.code,-32020);
  assert.equal(validateHttpMcp(msg,{...base,'mcp-session-id':'legacy-session'}).body.error.code,-32020);
...
test('unsupported protocol version fails explicitly',()=>{
  const msg={jsonrpc:'2.0',id:1,method:'tools/list',params:{_meta:{'io.modelcontextprotocol/protocolVersion':'2099-01-01'}}},r=validateHttpMcp(msg,{'mcp-protocol-version':'2099-01-01'});
