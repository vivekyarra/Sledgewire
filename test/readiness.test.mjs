...
test('readiness does not require a daemon when no Arena Room is configured',()=>{
  const s=new ArenaStore(':memory:');assert.deepEqual(readArenaDaemonReadiness(s,null),{required:false,ready:true,status:'not_required',age_ms:null});
});
...
test('fresh running daemon heartbeat is ready and stale heartbeat fails closed',()=>{
  const s=new ArenaStore(':memory:'),now=Date.now();writeArenaDaemonHeartbeat(s,room,{instanceId:'i_ABCDEFGHIJ',at:new Date(now-5000).toISOString()});
  let r=readArenaDaemonReadiness(s,room,{now,maxAgeMs:45_000});assert.equal(r.ready,true);assert.equal(r.status,'running');assert.equal(r.age_ms,5000);
