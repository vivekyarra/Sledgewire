...
function fakeApi(privateKeyPem,{spoof=false}={}){
  ...
  return {
    ...
    async wait(){
      if(phase==='quote'){
        phase='idle';
        const quote=signReceipt({type:'sledgewire.payment_required.v1',request_id:'rehearsal-fixed',service:'sledgewire.smoke',price_credits:3,payee,memo:'sledgewire:rehearsal-fixed:sledgewire.smoke',room_id:room,buyer_seat:buyer,issued_at:'2026-09-26T00:00:00.000Z'},privateKeyPem);
        const items=[];if(spoof)items.push(reply('msg_REQUEST001',{type:'sledgewire.payment_required.v1',request_id:'rehearsal-fixed',service:'sledgewire.smoke',price_credits:3,payee:'p_ATTACKERAA',memo:'sledgewire:rehearsal-fixed:sledgewire.smoke',room_id:room,buyer_seat:buyer}));
        items.push(reply('msg_REQUEST001',quote));return {items};
      }
...
test('live rehearsal ignores forged matching Room replies until an authentic signed seller reply arrives',async()=>{
  const kp=generateSigningKeypair(),api=fakeApi(kp.privateKeyPem,{spoof:true}),traceId='11111111-1111-4111-8111-111111111111';
  const trace=signReceipt({service:'sledgewire.trace',state:'READY',trace_id:traceId,events:[{event:'allow'}]},kp.privateKeyPem);
  const r=await runLiveSmokeRehearsal({api,roomId:room,payee,publicBaseUrl:'https://sledgewire.example',targetEndpoint:'https://target.example/mcp',publicKeyPem:kp.publicKeyPem,providerBootId:'123e4567-e89b-42d3-a456-426614174000',lookupTrace:async()=>trace,requestId:'rehearsal-fixed',timeoutMs:1000});
  assert.equal(r.verified,true);assert.equal(r.ignored_reply_candidates,3);assert.equal(api.stats().payCalls,1);assert.equal(api.stats().posts,3);
});
