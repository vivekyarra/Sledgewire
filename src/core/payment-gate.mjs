...
export class PaymentGate{
  constructor({ledger,store,prices,payee}){this.ledger=ledger;this.store=store;this.prices=prices;this.payee=payee;}
  async authorize(req,signal){
    ...
    const addressed=tx.addressed_to??tx.to;
    if(tx.payee_ok!==true&&addressed!==this.payee)return {ok:false,reason:'wrong_payee'};
    ...
    const fp=requestFingerprint(req);const claim=this.store.claim({requestId:req.requestId,txnId:req.txnId,fingerprint:fp,service:req.service});
    if(claim.status==='conflict')return {ok:false,reason:'transaction_or_request_reused'};
    if(claim.status==='inflight')return {ok:false,reason:'request_already_inflight',fingerprint:fp};
