...
export function validateSchemaValue(value,schema,path=[]){
  ...
  if(Array.isArray(type)){
    const failures=[];for(const candidate of type){const r=validateSchemaValue(value,{...schema,type:candidate},path);if(r.ok)return r;failures.push(r.reason);}
  ...
  if(type==='object'){
    ...
    for(const [k,v] of Object.entries(value)){
      if(props[k]){const r=validateSchemaValue(v,props[k],[...path,k]);if(!r.ok)return r;}
      else if(schema.additionalProperties===false)return {ok:false,reason:`additional_property:${k}`,path:[...path,k]};
      else if(schema.additionalProperties&&typeof schema.additionalProperties==='object'){const r=validateSchemaValue(v,schema.additionalProperties,[...path,k]);if(!r.ok)return r;}
    }
    return {ok:true};
  }
  if(type==='array'){
