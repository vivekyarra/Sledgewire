...
export async function invoke(endpoint,request,opts={}){
  ...
  const scan=scanUntrusted(result);return {service:'sledgewire.invoke',state:scan.suspicious?'DEGRADED':'READY',repair:prepared.repair,inspector:prepared.inspector,untrusted_tool_metadata:prepared.untrusted_tool_metadata,metadata_markers:prepared.metadata_markers,result_untrusted_content:scan.suspicious,result_markers:scan.hits,result,result_sha256:sha256(result)};
}
