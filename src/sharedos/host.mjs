...
export function createKernel(store){const kernel=new SharedOSKernel({grantSource:store,authorizer:new CapabilityAuthorizer({usageStore:store}),audit:store});
...
export async function runPaidService({service,input,store,requestId,fingerprint,buyerSeat}){const kernel=createKernel(store);const traceId=crypto.randomUUID();
  if(service!=='sledgewire.invoke'){
    const short=service.split('.').at(-1);const tool=`sledgewire.paid.${short}`;issue(store,grant({id:`g-${requestId}-dispatcher`,subject:ROLES.dispatcher,path:['services',short]}));const output=await invokeTool(kernel,context(ROLES.dispatcher,traceId),tool,input);return {service,state:output.state??'UNKNOWN',result:output,sharedos_trace_id:traceId,buyer_seat:buyerSeat,request_fingerprint:fingerprint};
  }
  ...
  issue(store,grant({id:`g-${requestId}-breaker`,subject:ROLES.breaker,path:['targets',h,request.name]}));const execution=await invokeTool(kernel,context(ROLES.breaker,traceId),'sledgewire.stage.breaker',{endpoint,name:request.name,arguments:repair.args});return {service,state:execution.state,result:{scout,repair,inspector,execution},sharedos_trace_id:traceId,request_fingerprint:fingerprint};
}
