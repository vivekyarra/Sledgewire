...
export function createArenaHandler({store,ledger,room,payee,signing,publicBaseUrl}){
  ...
  async function serve(req,buyerSeat){
    if(typeof req.request_id!=='string'||req.request_id.length<3||req.request_id.length>200)return failure(req,'invalid_request_id');
    ...
    if(!req.payment_txn_id){
      const price=prices[req.service];const memo=paymentMemo(req.request_id,req.service);
