...
console.error(JSON.stringify({sledgewire:'arena-daemon',room,instance:selfSeat,cursor,concurrency}));
let backoff=500;
for(;;){
  try{
    ...
    for(const message of items){if(store.roomMessageSeen(message.id)){cursor=Math.max(cursor,Number(message.sequence??cursor));store.setMeta(key,String(cursor));}else break;}
