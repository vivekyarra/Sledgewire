...
export function normalizeTransfer(tx,identity){
  ...
  return {
    id:asId(tx.id),
    buyer_instance_id:buyerInstance,
    sender_principal_id:senderPrincipal,
    recipient_principal_id:recipientPrincipal,
    addressed_to:addressedTo,
    amount:Number(tx.amount),
    room_id:asId(tx.room_id),
    memo:typeof tx.memo==='string'?tx.memo:null,
    payee_ok:Boolean(incoming),
    raw_kind:asId(tx.kind??tx.type)
  };
}
...
export class SharedNetApi{
  ...
  async request(path,{method='GET',body=null,idempotencyKey=null,signal=null}={}){
    ...
    try{
      ...
      if(!r.ok)throw new Error(`sharednet_http_${r.status}:${payload?.error?.code??'unknown'}`);
      return payload;
  ...
  async join(roomId,signal){if(!ROOM.test(roomId))throw new Error('invalid_sharednet_room');return this.request(`/api/v1/rooms/${roomId}/join`,{method:'POST',idempotencyKey:crypto.randomUUID(),signal});}
  ...
  async get(txnId,signal){
    ...
    const identity=await this.current(signal);let before=null;
    for(let page=0;page<5;page++){
      const qs=new URLSearchParams({limit:'100'});if(before)qs.set('before',before);
      const x=await this.request(`/api/v1/credits/transfers?${qs}`,{signal});
      const item=(x?.items??[]).find(t=>t?.id===txnId);
      if(item)return normalizeTransfer(item,identity);
      if(!x?.has_more||!x?.next_cursor)break;
      before=x.next_cursor;
    }
    return null;
  }
...
export function parseWatchBatch(text){
  const x=JSON.parse(text);const messages=Array.isArray(x)?x:(Array.isArray(x?.messages)?x.messages:(x?.message?[x.message]:[]));
  if(!messages.length)throw new Error('invalid_sharednet_watch_batch');
  const roomId=x?.room_id??messages[0]?.room_id;
  if(!ROOM.test(roomId??''))throw new Error('invalid_sharednet_watch_room');
  for(const m of messages){if(typeof m?.content!=='string'||(m.room_id&&m.room_id!==roomId)||!SEAT.test(senderInstance(m)??''))throw new Error('invalid_sharednet_watch_message');}
