...
const store=new ArenaStore(dbPath);const api=new SharedNetApi();const signing=loadSigningMaterial({production:true});
...
for(const message of batch.messages){
  ...
  if(response){const delivery=await compactForRoom(api,room,message.id,response);responses.push({reply_to:message.id??null,response:JSON.parse(delivery.text)});}
}
