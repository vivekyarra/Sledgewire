...
const server=http.createServer(async(req,res)=>{
  ...
  if(req.method==='GET'&&req.url==='/health'){
    ...
    return json(res,200,{ok:true,name:'sledgewire',version:'0.3.8',key_id:PUBLIC_KEY_ID,active_requests:active,paid_execution:publicArena?'sharednet-payment-required':'direct-enabled',arena_daemon:daemon});
  }
  if(req.method==='GET'&&req.url==='/ready'){
    const daemon=readArenaDaemonReadiness(traceStore,arenaRoomId),ready=daemon.ready;
    return json(res,ready?200:503,{ready,name:'sledgewire',version:'0.3.8',key_id:PUBLIC_KEY_ID,paid_execution:publicArena?'sharednet-payment-required':'direct-enabled',arena_daemon:daemon});
  }
  ...
  if(req.method==='GET'&&req.url==='/.well-known/agent.json'){const card=arenaCard(base);return json(res,200,{name:'Sledgewire',description:card.one_line,version:'0.3.8',mcp_url:card.mcp_url,quickstart_url:card.quickstart_url,catalog_url:card.catalog_url,public_key_url:card.public_key_url,fastest_demo:card.fastest_demo,tools:toolDefs.map(x=>x.name)});}
