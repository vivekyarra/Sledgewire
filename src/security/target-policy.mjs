...
for(const [addr,prefix,family] of [
  ...
  ['::',128,'ipv6'],['::1',128,'ipv6'],['::ffff:0:0',96,'ipv6'],['64:ff9b::',96,'ipv6'],['64:ff9b:1::',48,'ipv6'],
  ['100::',64,'ipv6'],['2001::',32,'ipv6'],['2001:2::',48,'ipv6'],['2001:db8::',32,'ipv6'],['2002::',16,'ipv6'],
  ['fc00::',7,'ipv6'],['fec0::',10,'ipv6'],['fe80::',10,'ipv6'],['ff00::',8,'ipv6']
])blocked.addSubnet(addr,prefix,family);

export function isBlockedIp(ip){
  const family=net.isIP(ip);if(!family)return false;
  return blocked.check(ip,family===4?'ipv4':'ipv6');
}
export async function resolveTarget(raw,opts={}){
  const {allowHttp=false,allowPrivate=false}=opts;let url;
  ...
  if((url.hostname==='localhost'||url.hostname.endsWith('.localhost'))&&!allowPrivate)throw new Error('private_target_blocked');
  let answers;
  if(net.isIP(url.hostname))answers=[{address:url.hostname,family:net.isIP(url.hostname)}];
  else{answers=await dns.lookup(url.hostname,{all:true,verbatim:true});if(!answers.length)throw new Error('dns_no_answers');}
  ...
  const selected=normalized.find(a=>allowPrivate||!isBlockedIp(a.address));if(!selected)throw new Error('no_allowed_target_address');
