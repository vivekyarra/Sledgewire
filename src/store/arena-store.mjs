...
export class ArenaStore {
  ...
  claim({requestId,txnId,fingerprint,service}){
    ...
    try{
      const byReq=this.db.prepare('SELECT * FROM requests WHERE request_id=?').get(requestId);
      const byTxn=this.db.prepare('SELECT * FROM requests WHERE txn_id=?').get(txnId);
      const existing=byReq??byTxn;
      if(existing){this.db.exec('COMMIT');
        const exact=existing.request_id===requestId&&existing.txn_id===txnId&&existing.fingerprint===fingerprint&&existing.service===service;
      ...
      this.db.prepare(`INSERT INTO requests(request_id,txn_id,fingerprint,service,status,started_at) VALUES(?,?,?,?, 'inflight',?)`).run(requestId,txnId,fingerprint,service,new Date().toISOString());
      this.db.exec('COMMIT');return {status:'claimed'};
  ...
  fail(requestId,fingerprint,error){this.db.prepare(`UPDATE requests SET status='failed',error_json=?,completed_at=? WHERE request_id=? AND fingerprint=? AND status='inflight'`).run(JSON.stringify(error),new Date().toISOString(),requestId,fingerprint);}
  ...
  async load(context,signal){signal?.throwIfAborted?.();const rows=this.db.prepare('SELECT grant_json FROM grants WHERE namespace_id=? AND revoked_at IS NULL').all(context.namespaceId);const actor=JSON.stringify(context.actor),authority=JSON.stringify(context.authority);return rows.map(r=>JSON.parse(r.grant_json)).filter(g=>JSON.stringify(g.subject)===actor&&JSON.stringify(g.issuer)===authority);}
